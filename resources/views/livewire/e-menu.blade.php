                ...
                @else
                    ...
                    <div class="border-t border-stone-100 pt-4 mb-5">
                        ...
                        <div class="space-y-2 mb-4">
                            <label class="text-xs font-bold text-stone-500 uppercase tracking-wide">Pay With</label>
                            <div class="grid grid-cols-2 gap-2">
                                @foreach($paymentMethods as $pm)
                                    <button wire:click="$set('selectedPaymentMethod', {{ $pm->id }})" class="py-2.5 px-3 rounded-xl border text-sm font-semibold flex items-center justify-center gap-2 transition-all {{ $selectedPaymentMethod == $pm->id ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-stone-200 bg-white text-stone-600 hover:border-stone-300' }}">
                                        @if($pm->type === 'khqr')
        ...
        <div class="relative w-full bg-white rounded-t-3xl shadow-2xl max-h-[85vh] flex flex-col"
            ...
            @if(!empty($cart))
                <div class="p-4 border-t border-stone-100 bg-stone-50 rounded-t-3xl shrink-0">
                    ...
                    <div class="space-y-2 mb-4">
                        <label class="text-[11px] font-bold text-stone-500 uppercase tracking-wide">Pay With</label>
                        <div class="grid grid-cols-2 gap-2">
                            @foreach($paymentMethods as $pm)
                                <button wire:click="$set('selectedPaymentMethod', {{ $pm->id }})" class="py-2.5 px-2 rounded-xl border text-xs font-semibold flex items-center justify-center gap-1.5 transition-all {{ $selectedPaymentMethod == $pm->id ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-stone-200 bg-white text-stone-600' }}">
                                    @if($pm->type === 'khqr')
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm14 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"></path></svg>
                                    @else
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
